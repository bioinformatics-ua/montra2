...
var TagCloudWidget = function TagCloudWidget(widgetname, width, height, pos_x, pos_y){
    ...
    __init : function(gridster, parent){
        ...
        self.icon = '<i class="fas fa-fw fa-cloud"></i>';
        ...
        .done(function(data) {
            if(data.tags){
                ...
                for(var i=0;i<tags.length;i++){
                    self.content += '<a style="line-height: 25px; margin: 3px; display: inline-block;" data-event="'+tags[i].name+'" rel="'+tags[i].relevance+'">'+tags[i].name+'</a>';
                }
                self.content += "</div>";
