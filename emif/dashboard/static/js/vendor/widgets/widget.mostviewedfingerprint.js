...
var MostViewedFingerprintWidget = function MostViewedFingerprintWidget(widgetname, width, height, pos_x, pos_y){
    ...
    __init : function(gridster, parent){
        ...
        self.icon = '<i class="fas fa-fw fa-star"></i>';
        ...
        $.get("api/mostviewedfingerprint/"+community)
        .done(function(data) {
           self.content = '<table class="table">';
            if(data.mostviewed){
                ...
                for(var i=0;i<data.mostviewed.length;i++){
                    self.content += '<tr><td style="word-break: break-all;"><small><a href="c/'+document.getElementById("communityindicator").value+'/fingerprint/'+data.mostviewed[i].hash+'/1">'+data.mostviewed[i].name+ "</a></small></td><!--td>" + data.mostviewed[i].count+"</td--></tr>";
                }
            }
