...
var RecommendationsWidget = function RecommendationsWidget(widgetname, width, height, pos_x, pos_y){
    ...
    __init : function(gridster, parent){
        ...
        self.icon = '<i class="fas fa-fw fa-bullhorn"></i>';
        ...
        $.get("api/recommendations/"+community)
        .done(function(data) {
           self.content = '<table class="table">';
            if(data.mlt){
                ...
                if(data.mlt.length ==0){
                    self.content += '<tr><td style="text-align: justify;text-justify: inter-word;">There\'s no recommendations yet.<br /> Recommendations are based on the databases the user is subscribed to.</td></tr>'
                }
            }
