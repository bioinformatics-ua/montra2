...
(function($) {
    $.fn.boolrelwidget = function(options) {
        ...
        var funcs = {
            ...
            countOcurrences: function(partial_ident, group, parent_id, parent_size){
                if(group instanceof BooleanGroup){
                    var gv = group.variables;

                    for(var i=0; i < gv.length; i++){
                        var this_group = gv[i];
                        if(this_group instanceof BooleanTerminal){
                            if(this_group.id.indexOf(partial_ident) != -1){
                                ocurr_count.push(1);
                            }
                        }
                        else {
                            funcs.countOcurrences(partial_ident, this_group, group.id, gv.length);
                        }
                    }
                }
            },
            lookup: function(partial_ident, group, parent_id, parent_size){
                if(group instanceof BooleanGroup){
                    var gv = group.variables;
                    ...
                    for(var i=0; i < gv.length; i++){
                        ...
                        if(this_group instanceof BooleanTerminal){
                            if(this_group.id.indexOf(partial_ident) != -1){
                                console.log(ocurr_count);
                                if(ocurr_count.length == 1)
                                    result_memory = group.id;
                                else
                                    result_memory = parent_id;
                            }
                        }
                        else {
                            funcs.lookup(partial_ident, this_group, group.id, gv.length);
                        }
                    }
                }
            },
            partialLookup: function(partial_ident){
                result_memory=undefined;
                ocurr_count = [];
                funcs.countOcurrences(partial_ident, mastergroup, undefined);

                funcs.lookup(partial_ident, mastergroup, undefined);

                return result_memory;
            },
            pushWithDelegate: function(ident, rep, answer, delegate, parent_id, relation) {
                var bt = new BooleanTerminal(ident, rep, answer, delegate);
                console.log(bt);
                console.log(bt.toQuery());
                ...
                if ((settings.auto_add == true) && (block != null)) {
                    ...
                    if (mastergroup == null)
                        ...
                        if(parent_id){
                            ...
                            if(relation)
                                mastergroup.addById(parent_id, sliced.copy(), relation);
                            else
                                mastergroup.addById(parent_id, sliced.copy(), settings.default_relation);
                        }

                        else
                            mastergroup.addById(mastergroup.id, sliced.copy(), settings.default_relation);
                    }
                }
            ...
            draw: function() {
                ...
                if (!settings.hide_concepts) {
                    ...
                    for (i = 0; i < basic_blocks.length; i++) {
                        ...
                        little_boxes.push('" class="btn btn-default boolrelwidget-block-inner">');
                ...
                if (something instanceof BooleanTerminal) {
                    ...
                    if (counter % 2 == 0) {
                        if (!something.isSimple() && counter > 0) {
                            big_box.push('<div class="btn btn-default boolrelwidget-collapser boolrelwidget-collapsed" id="boolrelwidget-cl-');
                            big_box.push(something.id);
                            big_box.push('"><i class="icon-zoom-in"></i> Expand</div>');
                        }
                        big_box.push('<div class="btn btn-default boolrelwidget-odd boolrelwidget-query-box');
                    ...
                    if (!settings.view_only) {
                        ...
                        big_box.push(something.id);
                        if (counter % 2 == 0) {
                            big_box.push('" title="Drag other concepts here, to nest a relation with this concept. See help for more details." data-toggle="tooltip" class="btn btn-default boolrelwidget-query-dropper');
                    ...
                    if (!settings.view_only) {
                        if (counter != 0) {
                            if (counter % 2 == 0)
                                big_box.push('<div class="btn btn-default boolrelwidget-query-delete');
                            else
            ...
            operator_selectbox: function(something, selected, counter) {
                ...
                if (settings.view_only)
                    select.push('" class="form-control boolrelwidget-select" disabled="disabled">');
                else
                    select.push('" class="form-control boolrelwidget-select">');
            ...
            reset: function() {
                if (settings.hide_concepts) {
                    if(mastergroup != null){
                        mastergroup = new BooleanGroup();

                        for (var j = 0; j < used_blocks.length; j++) {
                            used_blocks[j].callDelegate();
            ...
            opAll: function(func) {
                ...
                // First we reset
                    if(mastergroup != null){
                        ...
                        for (var j = 0; j < used_blocks.length; j++) {
                            mastergroup.addBoolean(func, used_blocks[j].copy());
                            //mastergroup.variables.push(used_blocks[j].copy());
                        }
        ...
        toolbar_content += '</li><li class="boolrelwidget-menu"><div class="boolrelwidget-arrow-r"><div class="boolrelwidget-arrow-down"></div></div></li></ul><div id="boolrelwidget-panel"><div id="boolrelwidget-basicblocks-out"><strong>Unused Terms</strong><div id="boolrelwidget-basicblocks" class="well well-small">Loading...</div></div><div class="clearfix" style="width: 90%">';


        if (!settings.view_only) {
            toolbar_content += '<div class="boolrelwidget-menu pull-left" data-original-title="Set of keywords and relations which generate search results" title="Set of keywords and relations which generate search results"><strong>Boolean Query</strong></div><div class="pull-right boolrelwidget-menu btn-group">';

            toolbar_content += '<button type="button" class="btn btn-default" data-toggle="modal" data-target="#boolrelwidgethelper"> <i class="icon-question-sign"></i> Help</button>';

            if (settings.form_anchor) {
                toolbar_content += '<button id="boolrelwidget-search" class="btn btn-default">Search</button>';
            ...
            toolbar_content += '<button id="boolrelwidget-collapseall" class="btn btn-default">Expand All</button><button class="btn btn-default" id="boolrelwidget-orall">Or All Concepts</button><button class="btn btn-default" id="boolrelwidget-andall">And All Concepts</button><button class="btn btn-default" id="boolrelwidget-clear">Reset</button></div>';
        } else {
            if(!settings.norefine){
                toolbar_content += '\
                    <button style="margin-right: 15px;margin-top: 5px; margin-bottom: 5px;" onclick="window.location.replace(\'' +
                    settings.link_back + '\'); return false;" class="pull-right btn btn-default">Refine Search</button>\
                ';
            }
        }
        if(settings.extra_buttons){
            toolbar_content += settings.extra_buttons;
        }
        if(settings.inplace){
            toolbar_content += '</div><div id="boolrelwidget-query">Loading...</div></div>';
        } else {
            toolbar_content += '</div><div class="col-md-12"><div class="panel panel-default"><div id="boolrelwidget-query" class="panel-body">Loading...</div></div></div></div>';
        }

        toolbar_content += '<!-- Modal --><div id="boolrelwidgethelper" class="boolrelwidgethelpercontainer modal fade" tabindex="-1" role="dialog" aria-labelledby="boolrelwidgethelperLabel" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><div class="modal-header">    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>    <h3 id="boolrelwidgethelperLabel">Help</h3>  </div>  <div class="boolrelwidgethelper modal-body">' + settings.help + '</div>  <div class="modal-footer">    <button class="btn btn-default" data-dismiss="modal" aria-hidden="true">Close</button></div></div></div></div>'
