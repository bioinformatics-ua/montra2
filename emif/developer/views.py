...
from community.models import Community, PluginPermission
        ...
        return render(request, self.template_name,
            {
                ...
                'plugin_groups': Plugin.all(owner=request.user).values('plugin_group').order_by('plugin_group').distinct('plugin_group'),
                'add': add,
                'update': update,
                'error': error,
                'developer': True
...
class DeveloperPluginSaveView(TemplateView):
    ...
    def post(self, request):
        ...
        index = request.POST.get('index', 1000)
        if index == "": index = 1000
        plugin_type = request.POST.get('type', None)
        icon = request.FILES.get('icon', None)
        remove_icon = request.POST.get("remove_icon", "no") == "yes"
        group_index = request.POST.get('group_index', 1000)
        if group_index == "": group_index = 1000
        plugin_group = request.POST.get('group', "")
        view_type = request.POST.get('iframe_type', 0)
...
class CommunityDeveloperIframeView(TemplateView):
    template_name = "developer_iframe.html"

    def get(self, request, community, plugin_hash):
        plugin = version = None
        ...
        obj = type('obj', (object,), {'plugin' : plugin})
        #if PluginPermission.check_permission(comm, request.user, plugin) or comm.filter(owners__id=request.user.id) > 0:

        hasPermission = False
        if PluginPermission.check_permission(comm, request.user, obj):
            hasPermission = True
        else:
            print "User does not have access to plugin"
            raise Http404
        
        if comm.owners.filter(id=request.user.id) > 0:
            print "Community manager accessing plugin..."
        else:
        ...
        if plugin.plugin_group:
            template_context.update({
                "activemenu": "plugin-group-{}".format(plugin.plugin_group),
                "activesubmenu": "plugin-{}".format(plugin_hash),
            })
        else:
            template_context.update({
                "activemenu": "plugin-{}".format(plugin_hash),
            })

        return render(request, self.template_name, template_context)

class CommunityDeveloperGlobalView(TemplateView):
    template_name = "developer_global.html"

    def get(self, request, community, plugin_hash):
        plugin = version = None
        try:
            plugin = Plugin.objects.get(name=plugin_hash)
            version = plugin.getLatest()
        except Plugin.DoesNotExist:
            pass
