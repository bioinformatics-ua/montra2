...
def sendCommitEmails(p, pv):
    ...
    try:
        send_custom_mail(config.brand+': Plugin %s v.%s submitted for approval' % (p.name, pv.version),
            """Dear Colleague,\n
                    A new plugin version %s for plugin %s has been submitted for approval on the platform <a href="http://bioinformatics.ua.pt/ehden">%s</a>.
                    ...
                    \n\n
                    \n\nSincerely,\n%s
            """ %(pv.version, p.name, config.brand, desc, config.brand), settings.DEFAULT_FROM_EMAIL, emails)
    except:
