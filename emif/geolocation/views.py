...
def geo(request, template_name='geo.html'):
    return geo_comm(request, None , template_name)

def geo_comm(request, community, template_name='geo.html'):
    ...
    if(request.session.get('isAdvanced') == True):
        ...
        if(request.session.get('query') != None):
            ...
            query = "text_txt:"+query

            if comm:
                query += ' AND %s' % comm.getSolrSlugs()

        else:
            if comm:
                query = comm.getSolrSlugs()
            else:
                query = "*:*"
        ...
        if _loc!= None and g != None and len(_loc)>1:
            ...
            # if dont have this city on the db
                ...
                if city != None:
                    ...
                    print "--[views] Error: retrieving geolocation"
                    continue
            ...
            def db_ready(database, city):
                return {    'name': database.name,
                            ...
                            'number_patients': str(database.number_patients),
