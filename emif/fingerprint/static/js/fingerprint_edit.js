...
$(function(){
    $('#draft_status').click(function(){
        const draft = $(this).is(':checked');
        $.post('fingerprint/api/draft/'+global_fingerprint_id, {
            draft: draft
        }).done(function(data) {
            if(!data.status){
                $("#database-message").show();
                setTimeout(function(){
                    $("#database-message").hide();
                },2000);
            }
            else{
                $("#draft-message").show();
            ...
            bootbox.alert('Could not change draft status, service failed, please contact the system administrator.');
        });
    });
});
