...
(function($) {
    $.fn.tabmanager = function(options) {
        ...
        var bodies = self.children('.panel').children('.panel-body').children('.tab-content');
        console.log(bodies);
        ...
        var private_funcs = {
            ...
            __renderRegistry: function(){
                if(settings.showRegistry != null && settings.registryTarget != null){
                    ...
                    var to_render = '<a class="btn btn-default dropdown-toggle" data-toggle="dropdown" href="#" id="add_list_toolbar"><i class="icon-retweet"></i> Add Tabs <span class="caret"></span></a>'+
        ...
        var tabcontrol = {
            init: function(){
                //header.children('li').addClass('old-move');
            ...
            addLayoutTab: function(id, head, body, pos){
                ...
                if(!inserted){
                    ...
                    if(header.children('li').length == 1 && has_content[id]){
                ...
                var hchildren = $(header.children('li').detach());

                hchildren.sort(function(a,b){
                    var an = parseInt($(a).data('pos')),
                        bn = parseInt($(b).data('pos'));
...
var TabWidget = function TabWidget(widgetname, header, pos, icon) {
    ...
    __init  :   function(tabcontrol, parent){
        ...
        '&nbsp;&nbsp;</div>'
        //'<div class="pull-right removewidget"><i class="icon-remove"></i>
        +'</div>';
        ...
        globalPluginsIds[this.header] = self.widgetname;
        ...
        if(has_content[this.widgetname] == false){
            $('a[href="#'+this.widgetname+'"]')[0].removeAttribute('data-toggle');
            $('a[href="#'+this.widgetname+'"]')[0].removeAttribute('href');
            $('li[data-id="'+this.widgetname+'"]').addClass('disabled');
        }
    },
    __refresh    : function(){
