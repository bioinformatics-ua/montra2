...
OpenButtonValidator.prototype = {
    ...
    validate : function(question_number, controllerDOM){
        const draw_validator = this.context.draw_validator;
        question_number = question_number.replace(".","\\.");
        const text = $(controllerDOM).val();

        //getValidator
        const validator = $('[id="open-button_validator_'+question_number+'"]');
        ...
        //get community slug and questionnaire from controller data attribute
        const comm_slug = $(controllerDOM).data("community")
        const questionnaire_slug = $(controllerDOM).data("questionnaire")

        $.ajax({
            type: 'GET',
            url: `api/validate?name=${text}&community=${comm_slug}&questionnaire=${questionnaire_slug}`,
            dataType: 'json',
            success: function(data) {
                if ("invalid_characters" in data) {
                    ...
                    draw_validator(
                        validator,
                        validated,
                        `The following character(s) are not allowed: ${data["invalid_characters"].join(" ")}`,
                    );
                    return;
                }
...
Fingerprint_Validator.prototype ={
    ...
    draw_validator: function(validator, validated, feedback_message){
        if (validated)
        {
            validator.removeClass("has-error");
            validator.addClass("has-success");
            $("span", validator).text(feedback_message);
        } else {
            validator.removeClass("has-success");
            validator.addClass("has-error");
            $("span", validator).text(feedback_message);
        }
    },
    setDatabase : function(db){
