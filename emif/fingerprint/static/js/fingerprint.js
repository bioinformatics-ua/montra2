...
var getPlacement = function($el) {
    var offset = $el.offset(),
        ...
        horizPlacement = horiz > 0 ? 'bottom' : 'left', // right overlaps with right hand side tab (see #1240)
...
function validate1(element, id_answered, dirty_id_answered, self_validated) {
    ...
    if ( self_validated === 1 || ($(element).val() != "" && self_validated === undefined)) {
        ...
        $('[id="answered_' + id_answered + '"]').removeClass('hide');
        ...
        if (!(typeof bool_container === 'undefined')) {
            ...
            if ($(':input[name="question_' + dirty + '"]').is(':radio')) {
                bool_container.pushWithDelegate('question_nr_' + number_correct,
                    ...
                    $(':input[name="question_' + dirty + '"]').filter(':checked').val(), 'clear_selection("question_nr_' + dirty + '", "");');
        ...
        $('[id="answered_' + id_answered + '"]').addClass('hide');
...
function validateById(id_answered, id_answered_aux) {
    ...
    /*
        ...
        1 - open | open-button | open-upload-image | open-textfield | datepicker | range | timeperiod | publication
            choice-yesno | choice-yesnocomment | choice-yesnodontknow | location | open-multiple | open-multiple-composition
    ...
    */
    ...
    //Added new types here
    if (qc_id.hasClass('type_open') || qc_id.hasClass('type_open-validated') ||  qc_id.hasClass('type_open-location')
        || qc_id.hasClass('type_open-multiple') || qc_id.hasClass('type_open-multiple-composition') || qc_id.hasClass('type_email') || qc_id.hasClass('type_url') ||
        qc_id.hasClass('type_numeric') || qc_id.hasClass('type_location')
        ...
        if (myValue != undefined) {
            ...
            if(qc_id.hasClass('type_open-validated')){
                ...
                if(comp){
                    $('[id="answered_' + id_answered_aux + '"]').removeClass('hide');
                    $('[id="answered_' + id_answered_aux + '"]').addClass("hasValue");

                    self_validated = 1;
                } else {
                    $('[id="answered_' + id_answered_aux + '"]').addClass('hide');
...
$(document).ready(function() {
    ...
    $(document).on('change', '.answer input, .answer select,.answer textarea', function(e) {
        ...
        if($(this).hasClass('commentary')){
            ...
            const $commentary = $('#commentary_' + qid);
            if (!is_empty) {
                $commentary.html('<i title="Click to open comments" class="fa fa-comment tooltippable">&nbsp;</i>');
            } else {
                $commentary.html('<i title="Click to open comments" class="fa fa-comment-o tooltippable">&nbsp;</i>');
            }
            return;
        }
