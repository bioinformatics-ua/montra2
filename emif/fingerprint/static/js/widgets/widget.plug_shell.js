...
var __loadLibs = function(array,callback, i){
    ...
    var lib = array[i];
    console.log(i);
    if(lib.indexOf('{% get_static_prefix %}') != -1){
        lib = lib.replace('{% get_static_prefix %}', base_url);
    }
    if (lib.match(".js$")) {
...
var PlugShellWidget = function PlugShellWidget(confs, show){
    if(confs){
        PlugShellWidget._base.apply(this, [confs.id, confs.name, confs.pos, confs.icon]);
    }
    else{
        PlugShellWidget._base.apply(this, [undefined, undefined, undefined]);
        confs = {};
    }
    this.show = show;
    ...
    refresh: function(){
        return PlugShellWidget._super.__refresh.apply(this);
    },
    ...
    container: function(){
        return '#'+this.widgetname+' .tab-body';
    },
    copy: function(){
