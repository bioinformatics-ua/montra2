...
############################################################
##### Geo names proxy (TO BE REPLACED BY SELF-HOSTED SERVICES WHEN READY) - Web service
############################################################

class GeonamesView(APIView):
    authentication_classes = (SessionAuthentication, BasicAuthentication)
    permission_classes = (IsAuthenticated,)
    def get(self, request, *args, **kw):
        ''' When the request is via get, we return all the questionnaire wizards missing handling
        ...
        if request.user.is_authenticated():
            ...
            for param in request.GET:
                if the_str == None:
                    the_str = '?%s=%s' % (param, request.GET[param])
                else:
                    the_str += '&%s=%s' % (param, request.GET[param])

            response = urllib2.urlopen('http://geotree.geonames.net/childrenJSON%s' % the_str)
            html = response.read()

            response = HttpResponse(html, status=status.HTTP_200_OK)
