...
def next_free_slug(slug_str, create=True, scope=None):
    ...
    while i < 1000:
        ...
        if len(slug) == 0:
            ...
            if scope != None:
                inscope = Question.objects.filter(questionset__in=scope.questionsets_ids(), slug_fk__slug1=this_try)
...
class TabularProcessor:
    def __init__(self, question):
        ...
        try:
            ...
            self.__x = mdata['data']['x']
            self.__y = mdata['data']['y']

            try:
                self.__type = mdata['data']['type']
            except:
    ...
    def ___add_if_not_exists(self, array, value):
    ...
    def append(self, values):
        if len(values) > 0:
            ...
            if len(parts) == 3:
                ...
                for val in x:
                    self.__x = self.___add_if_not_exists(self.__x, val.strip())

                for val in y:
                    self.__y = self.___add_if_not_exists(self.__y, val.strip())
    ...
    def __unicode__(self):
        if len(self.x()) > 0 or len(self.y()) > 0:
            return '%s\\\\%s\\\\%s' % ('|'.join(self.x()), '|'.join(self.y()), self.__type)
