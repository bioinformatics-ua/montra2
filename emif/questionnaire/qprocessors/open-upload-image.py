...
@question_proc('open-upload-image')
def question_(request, question, filepath=None):
    ...
    if filepath:
        ...
        value = request.POST.get('question_%s' % question.number)

    return {
        'required' : question.getcheckdict().get('required', False),
        'value' : value,
        "hasValue": value!= "",
        'template' : 'questionnaire/open-upload-image.html',
    }


@answer_proc('open-upload-image')
...
add_type('open-upload-image', 'open-upload-image')
