...
from questionnaire import add_type, answer_proc, question_proc, show_summary, AnswerException
...
@show_summary('choice-yesnodontknow', 'choice-yesno')
...
def show_summ_textfield(value, question=None):
    return value


@show_summary('email')
def show_summ_url(value, question=None):
    return '<a href="mailto:{}">{}</a>'.format(value, value)


@show_summary('url')
def show_summ_url(value, question=None):
    return '<a target="_blank" rel="noopener" href="{}">{}</a>'.format(value, value)
