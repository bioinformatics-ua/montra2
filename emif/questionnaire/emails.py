...
def send_emails(request=None, qname=None):
    ...
    questionset = questionnaire.questionsets().order_by('sortid')
    if not questionset:
    ...
    runinfos = RunInfo.objects.filter(subject__formtype='email', questionset_in=questionnaire.questionsets_ids())
